...
function plugin:print(...)
	...
	local color = self.nameSpace == "compiled/modes" and 248 or self._printColor
...
function plugin:setConfig()
	...
	-- Load config
	local conf = config[fixNameSpace(self.nameSpace)][self.fileName]
	if conf then
...
function fixNameSpace(ns)
	if ns == "compiled/plugins" then
		return "plugins"
	elseif ns == "compiled/modes" then
		return "modes"
	end
	...
	for _, entry in ipairs(entries) do
		if (entry.isDirectory or entry.extension == ".lua") and not hook.plugins[entry.stem] then
			...
			if entry.isDirectory then
				plug.entryPath = nameSpace .. "/" .. entry.stem .. "/index.lua"
			...
			printScoped(string.format("Loading \27[30;1m%s.\27[0m%s", fixNameSpace(nameSpace), entry.stem))
...
local function loadPluginNameSpace(nameSpace, isEnabledFunc)
	printScoped("Loading " .. fixNameSpace(nameSpace) .. "...")

	local numLoaded = discoverInNameSpace(nameSpace, isEnabledFunc)
	printScoped("Loaded " .. numLoaded .. " " .. fixNameSpace(nameSpace))
...
local function loadPlugins()
	...
	loadPluginNameSpace("compiled/plugins", shouldStartPluginEnabled)
	loadPluginNameSpace("compiled/modes", shouldStartModeEnabled)
...
function discoverNewPlugins()
	return discoverInNameSpace("compiled/plugins", shouldStartPluginEnabled)
		+ discoverInNameSpace("compiled/modes", shouldStartModeEnabled)
end
